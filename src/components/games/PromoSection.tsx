...
function pts(p: Promo) { return p.rewardPoints ?? p.reward_points ?? 0; }
function maxC(p: Promo) { return p.maxClaims ?? p.max_claims ?? 0; }
function totC(p: Promo) { return p.totalClaimed ?? p.total_claimed ?? 0; }
...
export default function PromoSection() {
  ...
  useEffect(() => { loadPromos(); }, [user]);

  async function loadPromos() {
    if (!user) return;
    const data = await fetch('/api/promos').then(r => r.json()).catch(() => []);
    setPromos((data as Promo[]).filter(p => totC(p) < maxC(p) && !claimed.has(p.id)));
  }
  ...
  async function handleClaim(promo: Promo) {
    ...
    try {
      if (typeof window !== 'undefined' && (window as any).Adsgram) {
        ...
        } catch { /* Ad failed/skipped, still allow claim */ }
      }

      const result = await claimPromo(promo.id, user.id);
      if (result.success) {
          ...
          <div className="relative flex items-center justify-between">
            <div>
              ...
              <div className="text-xs mt-1" style={{ color: 'hsl(var(--muted-foreground))' }}>
                🎁 {pts(promo)} pts • {maxC(promo) - totC(promo)} slots left
              </div>
            </div>
            <button
