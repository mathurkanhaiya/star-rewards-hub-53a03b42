...
/* ─────────────────────────────────────────────
Pure helpers — defined OUTSIDE component so
they’re never recreated on re-render
───────────────────────────────────────────── */
...
// Safe localStorage wrappers — won’t crash in private/incognito
...
/* ─────────────────────────────────────────────
CSS — constant outside component so React
never diffs/recreates the <style> node
───────────────────────────────────────────── */
const CSS = `
...
/* ── Energy — restored from localStorage on mount ── */
...
- Refs used INSIDE the master interval to avoid stale closures.
- Rule: whenever state changes, mirror it to the ref immediately.
  */
...
// ^ intentionally not listing the loaders to avoid a dep cycle;
//   they’re memoised and only depend on `user` anyway.

/* ──────────────────────────────────────────
MASTER INTERVAL — replaces 4 separate timers.
All state accessed via refs → zero stale-closure bugs.
────────────────────────────────────────── */
useEffect(() => {
...
```
  ...
  // 4. Farm progress (reads ref — no stale values)
...
}, []); // stable — accesses nothing from closure

/* ── creditBalance (memoised, parallel writes) ── */
...
```
  if (bal) {
    // Fire update + insert in parallel
    await Promise.all([
...
/* ── Tap flush — writes ALL batched taps in ONE DB round-trip ── */
...
```
  ...
  // Restore pending pts so they aren't silently lost
...
// Flush any remaining taps on unmount so no points are lost
useEffect(() => {
return () => {
...
/* ── handleTap — instant UI, debounced DB write ── */
const handleTap = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
// Read energy from ref → handleTap stays stable, not recreated every second
...
// Accumulate & debounce — no await, no lag
pendingTapPts.current += pts;
if (tapFlushTimer.current) clearTimeout(tapFlushTimer.current);
tapFlushTimer.current = setTimeout(flushTaps, 1500);
```

}, [user, flushTaps]); // energy NOT in deps — read via ref
...
```
  // Double-check server state to prevent double-claims
  ...
  // Treat unique-violation as "already claimed" — not an error
...
/* ── Derived (cheap, computed each render) ── */
